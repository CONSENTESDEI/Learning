...
>- **交叉验证**：
>
>  数据集的**分割方法**：划分成 n 份，1 份做**验证集**，n-1 份做**训练集**。分成几份叫**几折交叉验证**。
>
>  **原理**：若划分 cv=4：
>
>  1. 第一次：第1份做验证集，其余3份做训练集 → 评估1次
>  2. 第二次：第2份做验证集，其余3份做训练集 → 评估1次
>  3. 第三次：第3份做验证集，其余3份做训练集 → 评估1次
>  4. 第四次：第4份做验证集，其余3份做训练集 → 评估1次
>  5. 计算4次评估分数的**平均值**，作为**当前这组超参数**（如 `n_neighbors=5`）的模型得分
>  6. 若 `n_neighbors=5` 的平均分最高，则选定该超参数，使用**全部训练集**重新训练最终模型，最后用**测试集**对最终模型做评估
>  7. **核心结论**：交叉验证算出来的评分不是超参数本身，而是针对**当前设置的超参数取值**（如 `n_neighbors=5`）的评价指标，用于帮我们选出最优的超参数值。
>
>
>
>- **网格搜索**：找最优超参（如 `n_neighbors=5`）
>
>  模型有很多超参数
>
>  **每组超参都要交叉验证**
...
>  **模型调参的最优工具**
>
>  将 若干参数 传递给 网格搜索对象，自动完成不同超参组合的 模型训练、模型评估。最终只返回一组最优超参
>
>  解决超参的组合
>
>- 二者组合形成一个模型参数调优的方案
